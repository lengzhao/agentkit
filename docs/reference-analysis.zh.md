...
| **Approval** | `dsh-approval`, `dsh-tool-ask-user` | Extension `tool_call` + `ctx.ui.confirm` | P1 | ✅ `approval/auto-allow`、`auto-deny` + Platform Permission（CLI y/n）；`tool/ask-user`（见 [platform-interaction.zh.md](platform-interaction.zh.md)） |
...
| **Sandbox** | landlock / bwrap / seatbelt | 无内置，Operations 可替换 | P2 | ⏸ 暂缓（policy + approval 为主；真要做时再引入 cap，见 [roadmap](roadmap.zh.md#暂缓os-级沙箱)） |
...
20. **Sandbox** — 进程/文件系统隔离（⏸ 暂缓，见 [roadmap](roadmap.zh.md#暂缓os-级沙箱)）
