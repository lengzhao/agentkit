...
func TestResolveAgentIDPriority(t *testing.T) {
	...
	if err := session.SetGlobalAgentBind(ctx, ws, "global-agent"); err != nil {
		t.Fatal(err)
	}

	got, sess, glob, err := session.ResolveAgentID(ctx, store, ws, sid, "request-agent")
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := session.SetGlobalAgentBind(ctx, ws, ""); err != nil {
		t.Fatal(err)
	}
	got, _, _, err = session.ResolveAgentID(ctx, store, ws, sid, "request-agent")
