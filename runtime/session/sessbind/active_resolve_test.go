...
func TestResolveActiveSessionID(t *testing.T) {
	...
	got, err := sessbind.ResolveActiveSessionID(context.Background(), store, entry)
	if err != nil {
		t.Fatal(err)
	}
	if got != logical {
	...
	got, err = sessbind.ResolveActiveSessionID(context.Background(), store, agentkit.SessionID("cli:default"))
	if err != nil {
		t.Fatal(err)
	}
	if got != "cli:default" {
