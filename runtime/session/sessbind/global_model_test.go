...
func TestGlobalModelBind(t *testing.T) {
	...
	if got, err := sessbind.GlobalModelBind(ctx, ws, "assistant"); err != nil || got != "" {
		t.Fatalf("initial = %q err=%v", got, err)
	}
	if err := sessbind.SetGlobalModelBind(ctx, ws, "assistant", "global-model"); err != nil {
		t.Fatal(err)
	}
	if got, err := sessbind.GlobalModelBind(ctx, ws, "assistant"); err != nil || got != "global-model" {
...
func TestResolveEffectiveModelPriority(t *testing.T) {
	...
	got, sess, glob, err := sessbind.ResolveEffectiveModel(ctx, store, ws, sid, "assistant", "default-model")
	if err != nil {
		t.Fatal(err)
	}
	...
	got, _, _, err = sessbind.ResolveEffectiveModel(ctx, store, ws, sid, "assistant", "default-model")
