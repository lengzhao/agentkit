...
func TestResolveAgentIDPriority(t *testing.T) {
	...
	if err := sessstore.SetGlobalAgentBind(ctx, ws, "global-agent"); err != nil {
		t.Fatal(err)
	}

	got, sess, glob, err := sessstore.ResolveAgentID(ctx, store, ws, sid, "request-agent")
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := sessstore.SetGlobalAgentBind(ctx, ws, ""); err != nil {
		t.Fatal(err)
	}
	got, _, _, err = sessstore.ResolveAgentID(ctx, store, ws, sid, "request-agent")
