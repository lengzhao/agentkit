...
func TestParentSessionForDelegatePrefersOpenSession(t *testing.T) {
	...
	got, err := sessstore.ParentSessionForDelegate(ctx, guard, parentID)
	if err != nil {
		t.Fatal(err)
	}
	if got.ID() != parentID {
...
func TestLoadSessionPrefersOpenTurn(t *testing.T) {
	...
	got, err := sessstore.LoadSession(ctx, guard, id)
	if err != nil {
		t.Fatal(err)
	}
	if got.ID() != id {
...
func TestParentSessionForDelegateReentrantGetFailsWithoutOpenSession(t *testing.T) {
	...
	_, err = sessstore.ParentSessionForDelegate(context.Background(), guard, parentID)
	if err == nil {
