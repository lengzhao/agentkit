...
func TestLatestTodosUsesMostRecentSnapshot(t *testing.T) {
	...
	if err := sessstore.AppendTodoUpdate(ctx, sess, "a", []sessstore.Todo{
		{ID: "1", Title: "first", Status: sessstore.TodoPending},
		{ID: "2", Title: "second", Status: sessstore.TodoPending},
	}); err != nil {
		t.Fatal(err)
	}
	if err := sessstore.AppendTodoUpdate(ctx, sess, "a", []sessstore.Todo{
		{ID: "1", Title: "first", Status: sessstore.TodoDone},
		{ID: "2", Title: "second", Status: sessstore.TodoInProgress},
	}); err != nil {
		t.Fatal(err)
	}
	...
	if todos[0].Status != sessstore.TodoDone {
...
func TestFinishAfterIgnoresEarlierRuns(t *testing.T) {
	...
	if err := sessstore.AppendRunFinish(ctx, sess, "a", sessstore.RunFinishData{
	...
	if got := sessstore.FinishAfter(events, startSeq); got != nil {
	...
	if finish == nil || finish.Status != sessstore.FinishBlocked {
...
func TestRunStartSeqIgnoresContinuations(t *testing.T) {
	...
	if err := sessstore.AppendTurnContinue(ctx, sess, "a", sessstore.TurnContinueData{
		Segment: 1,
	...
	if got := sessstore.RunStartSeq(events); got != startSeq {
...
func TestRepeatedToolCallsCountsConsecutiveTail(t *testing.T) {
	...
	appendCall := func(name, input string) {
		t.Helper()
		if err := sessstore.AppendToolCall(ctx, sess, "a", agentkit.ToolCall{
	...
	if got := sessstore.RepeatedToolCalls(events, 0); got != 3 {
	...
	if got := sessstore.RepeatedToolCalls(events, 0); got != 1 {
...
func TestTotalUsageSumsAfterSeq(t *testing.T) {
	...
	ctx := context.Background()
	sess := newRunStateSession(t)
	if err := sessstore.AppendUsage(ctx, sess, "a", sessstore.UsageData{InputTokens: 10, OutputTokens: 5, TotalTokens: 15}); err != nil {
		t.Fatal(err)
	}
	events, err := derive.ReadAllEvents(ctx, sess)
	if err != nil {
	...
	if err := sessstore.AppendUsage(ctx, sess, "a", sessstore.UsageData{InputTokens: 20, OutputTokens: 7, TotalTokens: 27}); err != nil {
		t.Fatal(err)
	}
	events, err = derive.ReadAllEvents(ctx, sess)
	if err != nil {
	...
	if got := sessstore.TotalUsage(events, 0).TotalTokens; got != 42 {
		t.Fatalf("total usage = %d, want 42", got)
	}
	if got := sessstore.TotalUsage(events, cutoff).TotalTokens; got != 27 {
