...
func crashedSession(t *testing.T) agentkit.Session {
	...
	sess, err := sessstore.NewMemory(sessstore.MemoryConfig{ID: "test:crashed"})
	if err != nil {
		t.Fatal(err)
	}
	if err := sessstore.AppendTurnStart(ctx, sess, "coder"); err != nil {
		t.Fatal(err)
	}
	...
	if err := sessstore.AppendStepStart(ctx, sess, "coder", 0); err != nil {
		t.Fatal(err)
	}
	if err := sessstore.AppendMessage(ctx, sess, "coder", agentkit.EventAssistantMessage, agentkit.ModelMessage{
		Role: "assistant",
	...
	if err := sessstore.AppendToolResult(ctx, sess, "coder", agentkit.ToolResult{
		ID:      "call-a",
		Name:    "read",
		Content: "package a",
	}); err != nil {
...
func TestScanIncompleteIgnoresClosedTurns(t *testing.T) {
	...
	ctx := context.Background()
	sess, err := sessstore.NewMemory(sessstore.MemoryConfig{ID: "test:clean"})
	if err != nil {
		t.Fatal(err)
	}
	if err := sessstore.AppendTurnStart(ctx, sess, "coder"); err != nil {
	...
	if err := sessstore.AppendStepEnd(ctx, sess, "coder", 0); err != nil {
		t.Fatal(err)
	}
	if err := sessstore.AppendTurnEnd(ctx, sess, "coder", 1); err != nil {
		t.Fatal(err)
	}
	events, err := derive.ReadAllEvents(ctx, sess)
	if err != nil {
...
func TestRepairIsIdempotent(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		...
		if _, err := sessstore.RepairIncomplete(ctx, sess, incomplete); err != nil {
...
func TestDeriveKeepsRepeatedToolCallIDsPaired(t *testing.T) {
	...
	ctx := context.Background()
	sess, err := sessstore.NewMemory(sessstore.MemoryConfig{ID: "test:repeatids"})
	if err != nil {
		t.Fatal(err)
	}
