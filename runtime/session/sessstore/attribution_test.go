...
func userCtx(userID string) context.Context {
	return rctx.ApplyEnvelopeToContext(context.Background(), agentkit.TurnEnvelope{Actor: agentkit.ActorRef{UserID: userID}})
}
...
func TestSharedSessionReplaysStoredInjectPrefix(t *testing.T) {
	...
	id := rctx.SlackSessionIDForScope(sessstore.ScopeChannel, "C001", "", "U111")
	if other := rctx.SlackSessionIDForScope(sessstore.ScopeChannel, "C001", "", "U222"); other != id {
	...
	if err := sessstore.AppendMessage(userCtx("U111"), sess, "coder", agentkit.EventUserMessage, textMessage("user", "[meta sender_id=U111]\n改一下 README")); err != nil {
		t.Fatal(err)
	}
	if err := sessstore.AppendMessage(context.Background(), sess, "coder", agentkit.EventAssistantMessage, textMessage("assistant", "好的")); err != nil {
		t.Fatal(err)
	}
	if err := sessstore.AppendMessage(userCtx("U222"), sess, "coder", agentkit.EventUserMessage, textMessage("user", "[meta sender_id=U222]\n顺便跑一下测试")); err != nil {
		t.Fatal(err)
	}
...
func TestNoUserIDLeavesHistoryUntouched(t *testing.T) {
	...
	if err := sessstore.AppendMessage(context.Background(), sess, "coder", agentkit.EventUserMessage, textMessage("user", "列出目录")); err != nil {
		t.Fatal(err)
	}
	msgs, err := sess.DeriveMessages(context.Background())
	if err != nil {
...
func TestImageOnlyMessageDerivesPlaceholder(t *testing.T) {
	...
	if err := sessstore.AppendMessage(userCtx("U111"), sess, "coder", agentkit.EventUserMessage, msg); err != nil {
		t.Fatal(err)
	}
	msgs, err := sess.DeriveMessages(context.Background())
	if err != nil {
