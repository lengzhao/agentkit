...
func TestReopenedSessionContinuesSeqNumbering(t *testing.T) {
	...
	first := open()
	for i := 0; i < 3; i++ {
		if err := sessevents.Default.AppendStepStart(ctx, first, "a", i); err != nil {
			t.Fatal(err)
		}
	}
...
func TestStoreLRUCacheReloadsFromDisk(t *testing.T) {
	...
	dir := t.TempDir()
	ctx := context.Background()
	store, err := sessstore.NewStore(sessstore.StoreConfig{
		Dir:               ".",
		MaxCachedSessions: 2,
	}, sessstore.StoreDeps{Workspace: rtworkspace.Static(dir)})
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := sessevents.Default.AppendMessage(ctx, s1a, "agent", agentkit.EventUserMessage, agentkit.ModelMessage{
		Role:    "user",
...
func TestStoreHeldSessionSurvivesCacheEviction(t *testing.T) {
	...
	if err := sessevents.Default.AppendMessage(ctx, held, "agent", agentkit.EventUserMessage, agentkit.ModelMessage{
		Role:    "user",
...
func TestStoreEnsuresToolWorkDir(t *testing.T) {
	...
	dir := t.TempDir()
	store, err := sessstore.NewStore(sessstore.StoreConfig{Dir: "sessions"}, sessstore.StoreDeps{Workspace: rtworkspace.Static(dir)})
	if err != nil {
		t.Fatal(err)
	}
