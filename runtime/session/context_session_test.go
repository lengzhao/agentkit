...
func TestParentSessionForDelegatePrefersOpenSession(t *testing.T) {
	...
	inner, err := session.NewStore(session.StoreConfig{Dir: "."}, session.StoreDeps{Workspace: rtworkspace.Static(t.TempDir())})
	if err != nil {
		t.Fatal(err)
	}
...
func TestParentSessionForDelegateReentrantGetFailsWithoutOpenSession(t *testing.T) {
	...
	_, err = session.ParentSessionForDelegate(context.Background(), guard, parentID)
	if err == nil {
