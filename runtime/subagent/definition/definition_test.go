...
`)
	...
	t.Run("no frontmatter", func(t *testing.T) {
		...
		_, err := parse("plain.md", "just a prompt, no frontmatter\n")
...
func TestLoadDefinitionsFirstDirWins(t *testing.T) {
	...
	defs, err := load(context.Background(), ws, []string{"local:agents", "global:agents", "missing:dir"})
	if err != nil {
		t.Fatalf("load: %v", err)
	}
	if len(defs) != 2 {
...
func TestLoadDefinitionsNoDirs(t *testing.T) {
	...
	defs, err := load(context.Background(), dirWorkspace{}, []string{"local:agents"})
	if err != nil {
		t.Fatalf("load: %v", err)
	}
	if len(defs) != 0 {
