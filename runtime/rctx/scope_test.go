package rctx_test

import (
	"testing"

	"github.com/lengzhao/agentkit"
	"github.com/lengzhao/agentkit/runtime/rctx"
)

func TestApplyScope(t *testing.T) {
	t.Parallel()

	delivery := rctx.BuildDeliverySessionID("slack", "C001", "123.456", "U777")

	cases := []struct {
		name  string
		scope agentkit.SessionScope
		want  agentkit.SessionID
	}{
		{"channel", agentkit.SessionScopeChannel, "slack:C001"},
		{"thread", agentkit.SessionScopeThread, "slack:C001:t:123.456"},
		{"user", agentkit.SessionScopeUser, "slack:C001:u:U777"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := rctx.ApplyScope(delivery, tc.scope, "U777")
			if got != tc.want {
				t.Fatalf("ApplyScope(%q, %q) = %q, want %q", delivery, tc.scope, got, tc.want)
			}
		})
	}
}

func TestParseDelivery(t *testing.T) {
	t.Parallel()

	got := rctx.ParseDelivery("slack:C001:t:123.456:u:U777", "")
	if got.Platform != "slack" || got.Channel != "C001" || got.Thread != "123.456" || got.User != "U777" || !got.Routable {
		t.Fatalf("got %+v", got)
	}
}

func TestApplyScopePassthroughCLI(t *testing.T) {
	t.Parallel()

	id := agentkit.SessionID("cli:default")
	for _, scope := range []agentkit.SessionScope{agentkit.SessionScopeChannel, agentkit.SessionScopeThread, agentkit.SessionScopeUser} {
		if got := rctx.ApplyScope(id, scope, ""); got != id {
			t.Fatalf("scope %q changed cli id to %q", scope, got)
		}
	}
}

func TestParseScopeDefaultsToChannel(t *testing.T) {
	t.Parallel()

	for _, raw := range []string{"", "channel", "CHANNEL", "bogus"} {
		if raw == "bogus" {
			if got := rctx.ParseScope(raw); got != agentkit.SessionScopeChannel {
				t.Fatalf("ParseScope(%q) = %q, want channel", raw, got)
			}
			continue
		}
		if got := rctx.ParseScope(raw); got != agentkit.SessionScopeChannel {
			t.Fatalf("ParseScope(%q) = %q, want channel", raw, got)
		}
	}
	if got := rctx.ParseScope("thread"); got != agentkit.SessionScopeThread {
		t.Fatalf("got %q", got)
	}
	if got := rctx.ParseScope("user"); got != agentkit.SessionScopeUser {
		t.Fatalf("got %q", got)
	}
}

func TestBuildDeliverySessionID(t *testing.T) {
	t.Parallel()

	got := rctx.BuildDeliverySessionID("slack", "C001", "123.456", "U777")
	want := agentkit.SessionID("slack:C001:t:123.456:u:U777")
	if got != want {
		t.Fatalf("got %q want %q", got, want)
	}
}

func TestApplyScopeScheduleSideSessionIsOpaque(t *testing.T) {
	t.Parallel()

	side := agentkit.SessionID("schedule:agent-1:123456789")
	if got := rctx.ApplyScope(side, agentkit.SessionScopeChannel, ""); got != side {
		t.Fatalf("ApplyScope = %q, want opaque %q", got, side)
	}
}

func TestDeliveryWithUser(t *testing.T) {
	t.Parallel()

	delivery := rctx.BuildDeliverySessionID("slack", "C001", "123.456", "U111")
	got := rctx.DeliveryWithUser(delivery, "U222")
	want := rctx.BuildDeliverySessionID("slack", "C001", "123.456", "U222")
	if got != want {
		t.Fatalf("got %q want %q", got, want)
	}
}
