...
// Service indexes durable session logs and supports full-text search per tenant workspace.
type Service interface {
	// SyncSessions ingests or refreshes index entries from the sessions source
	// configured on the implementation (tenant-scoped via ctx).
	SyncSessions(ctx context.Context) error
