...
func TestMultiIsolatesJobsPerChannel(t *testing.T) {
	...
	ch1 := session.ApplyEnvelopeToContext(context.Background(), agentkit.TurnEnvelope{Route: session.SessionRoute("slack", "slack:C001"), Conversation: "slack:C001", Workspace: "slack:C001"})
	ch1 = session.ContextWithDeliveryRoute(ch1, "slack", agentkit.SessionID("slack:C001:u:U1"))
	ch2 := session.ApplyEnvelopeToContext(context.Background(), agentkit.TurnEnvelope{Route: session.SessionRoute("slack", "slack:C002"), Conversation: "slack:C002", Workspace: "slack:C002"})
