...
func TestNormalizeSessionIDBareChat(t *testing.T) {
	...
	ctx := rctx.ApplyEnvelopeToContext(context.Background(), agentkit.TurnEnvelope{Route: session.SessionRoute("feishu", "delivery")})
	if got := rtdelivery.NormalizeSessionID(ctx, "oc_a1b2c3d4e5"); got != "feishu:oc_a1b2c3d4e5" {
		t.Fatalf("feishu: got %q", got)
	}
	ctx = rctx.ApplyEnvelopeToContext(context.Background(), agentkit.TurnEnvelope{Route: session.SessionRoute("slack", "delivery")})
