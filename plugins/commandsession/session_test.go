package commandsession_test

import (
	"context"
	"strings"
	"testing"

	"github.com/lengzhao/agentkit"
	"github.com/lengzhao/agentkit/cap/command"
	"github.com/lengzhao/agentkit/plugins/commandsession"
	"github.com/lengzhao/agentkit/runtime/session"
)

func TestSessionCommandShowsInfo(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	store, err := session.NewStore(session.StoreConfig{Dir: dir})
	if err != nil {
		t.Fatal(err)
	}
	sessionID := agentkit.SessionID("cli:default")
	sess, err := store.Get(context.Background(), sessionID)
	if err != nil {
		t.Fatal(err)
	}
	if err := session.AppendMessage(context.Background(), sess, "coder", agentkit.EventUserMessage, agentkit.ModelMessage{
		Role:    "user",
		Content: []agentkit.ContentPart{{Type: "text", Text: "hello"}},
	}); err != nil {
		t.Fatal(err)
	}

	handler, err := commandsession.New(commandsession.Config{}, commandsession.Deps{SessionStore: store})
	if err != nil {
		t.Fatal(err)
	}
	var out strings.Builder
	_, err = handler.Handle(context.Background(), command.Request{
		Name:      "session",
		SessionID: sessionID,
		ErrOut:    &out,
	})
	if err != nil {
		t.Fatal(err)
	}
	text := out.String()
	for _, want := range []string{"session id: cli:default", "path:", "events:", "messages: 1"} {
		if !strings.Contains(text, want) {
			t.Fatalf("output missing %q: %s", want, text)
		}
	}
}
